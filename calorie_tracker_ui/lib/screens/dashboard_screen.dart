...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  // For the "Today's Progress" rings: prefer day-specific target if gym
  // flag is explicitly set for the selected date, else weekly average.
  bool get _selectedGymFlagSet =>
      logFor(_selectedDate).gymDay != null;
  bool get _selectedIsGymDay =>
      logFor(_selectedDate).gymDay?.didGym ?? false;

  double get _targetCalories {
    if (_selectedGymFlagSet) {
      return NutritionTargetEngine().dayTarget(
        _profile,
        isGymDay: _selectedIsGymDay,
        health: _syncResult,
      ).calories;
    }
    return _weeklyPlan.avgDailyCalories;
  }
  ...
  double get _targetProtein {
    ...
    return _weeklyPlan.avgDailyProtein;
  }
