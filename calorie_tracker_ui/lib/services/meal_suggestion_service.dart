...
class MealSuggestionService {
  ...
  List<MealSuggestion> suggestionsForDay({
    ...
    final todayWorkout = WorkoutService.instance.sessionFor(date);
  ...
  double _score(
    ...
    // Extra boost once workout is actually completed — recovery window matters.
    if (todayWorkout != null && !todayWorkout.isEmpty) {
      if (c.isProteinFocused) {
        score += 12;
      }
      if (todayWorkout.totalSets >= 10 &&
          c.caloriesKcal >= 180 &&
          !c.isCarbHeavy) {
        score += 6;
      }
  ...
  MealSuggestion _toSuggestion(
    ...
    final String reason;
    final todayWorkout = WorkoutService.instance.sessionFor(DateTime.now());
    ...
    if (sequencing.mode == _MealMode.proteinCorrection && c.isProteinFocused) {
      ...
      reason = todayWorkout != null && !todayWorkout.isEmpty
      ...
      reason = 'Good fit for what you are actually missing right now.';
        ...
        c.isProteinFocused &&
        c.caloriesKcal >= 180) {
      reason = 'Good recovery meal option after training.';
